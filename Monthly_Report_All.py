...
def createMetric(df, folder, brand):
    ...
    for a in df.columns:#Only select the date and monthly metrics
        if a.startswith('26 Week Average') | a.startswith('Date'):#26 Week Average. Can also use Monthly, Quarterly etc as well
...
dfbrand=pd.read_excel("Select_Brands_Metrics.xlsx",0,index_col=False)
dfbrand = dfbrand[dfbrand.Select==1]
dfbrand.reset_index(level=0, inplace=True)


dfmetric=pd.read_excel("Select_Brands_Metrics.xlsx",1,index_col=False)
...
#dfoutput = pd.melt(dfoutput, id_vars=['Brand','Metric', 'Region', 'Age', 'Economic_Segment'], value_vars=columns, var_name='Period', value_name='Metric_Value')

dfoutput = pd.melt(dfoutput, id_vars=['Brand','Metric', 'Region', 'Age', 'Economic_Segment'], value_vars=columns)

dfoutput.to_excel("New_Look_Competitor_Satisfaction.xlsx")
