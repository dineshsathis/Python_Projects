...
Focus_Brand = "New Look" #Update this before the program is run
Satisfaction_Driver = ['Product_Specific_CPM_quality','Product_Specific_CPM_durability','Product_Specific_CPM_range_of_colours','Product_Specific_CPM_range_of_styles','Product_Specific_CPM_ease_of_care','Product_Specific_CPM_offers_in_size_body_shape','Product_Specific_CPM_comfort','Product_Specific_CPM_stylish_on_trend','Product_Specific_CPM_suit_my_style','Product_Specific_CPM_ethical_position','Service_CPM_how_clean_tidy_store','Service_CPM_how_long_to_pay_queue','Service_CPM_how_busy_store','Service_CPM_availability_condition_fitting_rooms','Service_CPM_availability_items_in_stock_your_size','Service_CPM_how_easy_to_navigate_find_items_in_store','Service_CPM_helpfulness_knowledge_staff','Service_CPM_return_refund_policy','Service_CPM_convenience_store_location','Service_CPM_product_reviews','Service_CPM_images_videos_of_products','Service_CPM_text_description_of_products','Service_CPM_personalized_customer_service','Service_CPM_delivery_options','Service_CPM_shipping_cost_speed','Service_CPM_how_easy_to_navigate_find_items_on_website','Service_CPM_checkout_process_payment_options','Service_CPM_products_resembled_expectations','Prices_CPM_overall_prices_at_retailer','Prices_CPM_prices_at_cheaper_end_of_retailers_range','Prices_CPM_prices_at_premium_end_of_retailers_range','Prices_CPM_number_of_reduced_price_products_available','Prices_CPM_number_of_special_offers_and_discounts_offered','Prices_CPM_offers_targeted_directly_to_you','Prices_CPM_regular_price_of_products_not_on_sale','Prices_CPM_range_of_prices_of_products_budget_treat','Prices_CPM_sales_discounts_promotions_on_products']
Satisfaction_Result = ['In_store_satisfaction', 'Online_satisfaction', 'Purchase_frequency_instore', 'Purchase_frequency_online', 'Bought_Instore', 'Bought_Online']

#Query the competitor set with the required conditions. Update this before the program is run

#Query = "select * \
#from a_temp.DSS_Brand_Satisfaction_UK where Retailer in (10,11,17,26,29,41,63,64,66,73,74,82,86,88,98,103,109,116,120) and Age >= 18 and Age <= 34"

Query = "select * from a_temp.DSS_Brand_Satisfaction_UK where Retailer in (73) and Age >= 18 and Age <= 34"
print(Query)
df = gbq.read_gbq(query=Query, project_id="studied-radar-148816")

df = df[Satisfaction_Driver + Satisfaction_Result]

columns = {'Result','Driver','slope', 'intercept', 'r_value', 'p_value', 'std_err'}
reg_output_df = pd.DataFrame(columns=columns)

row = 0
for metric in range(0,len(Satisfaction_Result)):
    for column in range(0,len(Satisfaction_Driver)):
        qstr = Satisfaction_Result[metric] + " >= 0 & " + Satisfaction_Driver[column] + " >= 0"
        tf = df.query(qstr)
        column_pair = [Satisfaction_Result[metric], Satisfaction_Driver[column]]
        tf = tf[column_pair]
        tf = tf.apply(pd.to_numeric)
        print(Satisfaction_Result[metric] + " : " + Satisfaction_Driver[column])
        slope, intercept, r_value, p_value, std_err = linregress(tf[Satisfaction_Result[metric]],tf[Satisfaction_Driver[column]])
        reg_output_df.loc[row, 'Result'] = Satisfaction_Result[metric]
        reg_output_df.loc[row, 'Driver'] = Satisfaction_Driver[column]
...
reg_output_df.to_excel("reg_output_Satisfaction_Driver_CPMs_" + Focus_Brand + ".xlsx")