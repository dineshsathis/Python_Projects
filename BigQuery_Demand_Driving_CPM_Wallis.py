...
Demand_Usage = ['Purchase_frequency_instore','Purchase_frequency_online','Purchase_frequency_overall','Instore_spend','Online_spend','Overall_spend', 'Bought_Instore', 'Bought_Online', 'Bought_Overall']
...
for metric in range(0,len(Demand_Usage)):
    for column in range(0,len(Satisfaction)):
        qstr = Demand_Usage[metric] + " >= 0 & " + Satisfaction[column] + " >= 0" #This removes the -99 and -999 values from database
        tf = df.query(qstr)
...
reg_output_df.to_excel("reg_output_demand_driving_CPM_" + Focus_Brand + ".xlsx")
